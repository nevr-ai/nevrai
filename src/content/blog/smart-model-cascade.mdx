...
description: "Free Groq → exhausted → OpenRouter free → exhausted → FAIL. Как мы построили каскад моделей, который никогда не падает: free first, paid fallback, автоматическая эскалация."
date: 2026-04-09
...
Мы не отслеживаем релизы моделей. Система делает это сама.
...
Мы установили `max_cost_per_1m: 0.5` для extraction-каскада. Это значит: система допускает модели до 50 центов за миллион токенов. В реальности выбирает самые дешёвые ($0.10–0.20/M). Бесплатные идут первыми, платные — только когда все free исчерпаны.
...
Мы видим что происходит, но не вмешиваемся. Система адаптируется сама.
...
Три часа даунтайма научили нас одному: бесплатное — это прекрасно, но без fallback'а это бомба с таймером.
