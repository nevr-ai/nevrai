...
Мы нашли архитектуру, которая решает 95% случаев без единого LLM-вызова. Оставшиеся 5% — да, LLM, но точечно и дёшево.
...
Эмпирически мы пришли к двум порогам, и это ключевое архитектурное решение:
...
Почему именно эти числа? Мы прогнали 2000 пар сущностей из реальных проектов через ручную разметку и построили precision/recall кривую:
...
При 0.92 precision = 0.97, то есть из 100 автоматических мёржей ошибочны только 3. При этом recall уже 0.89 — мы ловим почти 9 из 10 реальных дубликатов. Ниже 0.92 precision падает быстро — слишком много false positives. Выше — теряем настоящие дубли.
...
1. **Exact match** по canonical key (lowercase, strip, transliterate). Бесплатно. Ловит "Customer Churn" = "customer churn" = "customer_churn". В наших данных это ~40% всех дубликатов.

2. **Cosine similarity** по эмбеддингам. Стоимость — один forward pass через embedding model (мы используем hash-based в dev, API в prod). Ловит "customer churn" = "user attrition" = "отток клиентов". Ещё ~45% дубликатов.
...
Между exact и semantic мы добавили Levenshtein с порогом расстояния ≤ 2 (для строк длиннее 5 символов). Это ловит опечатки, которые эмбеддинг-модель может не заметить:
...
Обратите внимание на кросс-языковые пары. Мультиязычные эмбеддинг-модели (мы используем bge-m3 от BAAI) отлично справляются с "unit economics" = "юнит-экономика", потому что эти термины встречаются в одних и тех же контекстах в обучающих данных.
...
Несколько вещей, которые мы узнали на практике:

- **Embedding drift**: одна и та же модель может давать слегка разные эмбеддинги после обновления. Мы храним версию модели вместе с эмбеддингом и перегенерируем при смене.

- **Короткие строки ненадёжны**: cosine similarity для "CRM" и "ERP" может быть 0.88. Для строк короче 4 символов мы снижаем порог автомёржа до 0.96.

- **Контекст важен**: "Apple" (компания) и "apple" (фрукт) имеют похожие эмбеддинги без контекста. Мы добавляем node_type в текст для эмбеддинга: "company: Apple" vs "product: apple".

- **Batch processing**: проверка каждой ноды против всех 500 — это O(n). Мы используем approximate nearest neighbor (ANN) индекс для ускорения поиска кандидатов перед точным cosine расчётом.
